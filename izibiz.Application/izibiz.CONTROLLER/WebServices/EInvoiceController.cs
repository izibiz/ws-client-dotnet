...
namespace izibiz.CONTROLLER.Web_Services
{
    public class EInvoiceController
    {
        ...
        public List<IncomingInvoices> getIncomingInvoice()
        {
            using (new OperationContextScope(EFaturaOIBPortClient.InnerChannel))
            {
                ...
                if (invoiceArray.Length > 0)
                {
                    ...
                    SaveInvoiceArrayToEntitiy(invoiceArray,context.IncomingInvoice);
                }
                //  return dbGetInvoices(EI.InvTableName.IncomingInvoices.ToString());
                return context.IncomingInvoice.ToList();
            }
        }

        private void SaveInvoiceArrayToEntitiy(INVOICE[] invoiceArray, DbSet<IncomingInvoices> entitiyInv)
        {
            foreach (var inv in invoiceArray)
            {
                IncomingInvoices invoiceMaster = new IncomingInvoices();
    ...
    public DataTable getDraftInvoice()
    {
        using (new OperationContextScope(EFaturaOIBPortClient.InnerChannel))
        {
            var req = new GetInvoiceRequest(); //sistemdeki gelen efatura listesi için request parametreleri
