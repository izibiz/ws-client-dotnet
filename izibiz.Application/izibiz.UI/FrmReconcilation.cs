...
namespace izibiz.UI
{
    public partial class FrmReconcilation : Form
    {
        ...
        private string reconcilationStatusDescWrite(string statusCode)
        {
            ...
            if (statusCode == "126")
            {
                return "MUTABIK DEÐÝL";
            }
            return "durum atamasý beklenýyor";
        }
        ...
        private string reconcilationMailStatusDescWrite(string mailStatusCode)
        {
            ...
            {
                return "E-POSTA GÖNDERÝM SONLANDI";
            }
            return "durum atamasý beklenýyor";
        }
        ...
        private void gridUpdateDespatchList(List<Reconcilations> gridListReconcilation)
        {
            ...
            {
                MessageBox.Show(Lang.noReconcilation);//"Getirilecek Mutabakat bulunamadý"
            }
            else
            {
                ...
                if (nameof(EI.Reconcilation.CM).Equals(reconcilationType)) //reconcý type CurrentReconcilation ise
                {
        ...
        private void ÝtemCurrentReconcilations_Click(object sender, EventArgs e)
        {
        ...
        private void ÝtemBaBsReconsilations_Click(object sender, EventArgs e)
        {
        ...
        private void ÝtemNewReconcilation_Click(object sender, EventArgs e)
        {
        ...
        private void BtnGetStatusReconcilation_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (uuidList.Count > 0) //gonderilerecek faturalar varsa
                {
                    ...
                    if (status != null)
                    {
                        MessageBox.Show(Lang.succGetStatus);//"Servisten durum sorgulama iþlemi basarýlý"

                        //db ye durumu gönderildi olarak guncelle
                        if (Singl.reconcilationDalGet.updateStatusReconcilation(status))

                            MessageBox.Show(Lang.succSaveStatus);//"Db ye  durum kaydetme basarýlý"

                        // guncel durumunu gostermek ýcýn tabloyu yenýleyelým
                        gridUpdateDespatchList(Singl.reconcilationDalGet.getReconcilationsWithType(reconcilationType));
                    }
                    else
                    {
                        MessageBox.Show(Lang.unSuccSaveStatus);//"Db ye mutaabakýn  durumunu kaydetme basarýsýz"
                    }
                }
                else
                {
                    MessageBox.Show(Lang.unsuccesChangeState, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);//"Servisten durum sorgulama iþlemi basarýsýz"
                }
            }
            catch (System.Data.Entity.Infrastructure.DbUpdateException ex)
            {
        ...
        private void BtnSendReconcilation_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataGridViewRow row in tableGrid.SelectedRows)
                {
                    if (Convert.ToBoolean(row.Cells[nameof(EI.Reconcilation.isSend)].Value))//daha once gonderýlmýs ise
                    {
                        beforeSendList.Add(row.Cells[nameof(EI.Reconcilation.uuid)].Value.ToString());
                    }
                    else  //gonderýlmemýsse
                    {
                ...
                if (beforeSendList.Count > 0) //onceden gonderýlmýs olanlar
                {
                    MessageBox.Show(string.Join(Environment.NewLine, beforeSendList) + Environment.NewLine + Lang.hasIdReconBeforeSending);//nolu mutabak daha once gonderýlmýs tekrar gonderemezsýnýz
                }
                ...
                if (validUuidList.Count > 0) //gonderilerecek faturalar varsa
                {
                    ...
                    {
                        MessageBox.Show(Lang.successLoad);//"Servise yukleme iþlemi basarýlý"
                        //db ye durumu gönderildi olarak guncelle
                        if (Singl.reconcilationDalGet.updateReconcilationIsSend(validUuidList, true))
                        {
                            MessageBox.Show(Lang.succSaveStatus);//"Db ye mutaabakýn gonderýlme durumunu kaydetme basarýlý"

                            //isSend in guncel durumunu gostermek ýcýn tabloyu yenýleyelým
                            gridUpdateDespatchList(Singl.reconcilationDalGet.getReconcilationsWithType(reconcilationType));
                        }
                        else
                        {
                            MessageBox.Show(Lang.unSuccSaveStatus);//"Db ye mutaabakýn gonderýlme durumunu kaydetme basarýsýz"
                        }
                    }
                    else
                    {
        ...
        private void btnSendMailReconcilation_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (uuidList.Count > 0) //gonderilerecek faturalar varsa
                {
                    ...
                    if (errorMessage == null)
                    {
                        MessageBox.Show(Lang.succSendMail);//"mail gönderme iþlemi basarýlý"
                    }
                    else
                    {
                        MessageBox.Show(errorMessage, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
